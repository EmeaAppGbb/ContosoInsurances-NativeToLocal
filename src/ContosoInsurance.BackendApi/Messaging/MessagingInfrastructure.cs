...
namespace ContosoInsurance.BackendApi.Messaging;
    ...
    public async Task DispatchPendingAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var pendingMessage in pendingMessages)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(ex, "Failed to publish outbox message {MessageId}", pendingMessage.MessageId);
            }
        }
...
public sealed class OutboxPublisherService(IServiceScopeFactory scopeFactory, ILogger<OutboxPublisherService> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Backend API outbox publisher iteration failed");
            }
...
public sealed class RabbitMqTopologyInitializer(IConnection rabbitConnection, ILogger<RabbitMqTopologyInitializer> logger) : IHostedService
{
    ...
    {
        await using var channel = await rabbitConnection.CreateChannelAsync(cancellationToken: cancellationToken);
        await channel.ExchangeDeclareAsync(MessagingTopology.CommandsExchange, ExchangeType.Topic, durable: true, autoDelete: false, cancellationToken: cancellationToken);
        await channel.ExchangeDeclareAsync(MessagingTopology.EventsExchange, ExchangeType.Topic, durable: true, autoDelete: false, cancellationToken: cancellationToken);
        await channel.ExchangeDeclareAsync(MessagingTopology.DeadLetterExchange, ExchangeType.Topic, durable: true, autoDelete: false, cancellationToken: cancellationToken);
        logger.LogInformation("Backend API RabbitMQ topology initialized");
    }
