...
namespace ContosoInsurance.Worker.Claims.Messaging;
    ...
    public async Task DispatchPendingAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var pendingMessage in pendingMessages)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(ex, "Claims worker failed to publish outbox message {MessageId}", pendingMessage.MessageId);
            }
        }
...
public sealed class OutboxPublisherService(IServiceScopeFactory scopeFactory, ILogger<OutboxPublisherService> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Claims worker outbox dispatch iteration failed");
            }
...
public sealed class RabbitMqTopologyInitializer(IConnection rabbitConnection, ILogger<RabbitMqTopologyInitializer> logger) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        await DeclareQueueAsync(channel, MessagingTopology.PrivateClaimIntakeQueue, RoutingKeys.ClaimSubmittedV1, cancellationToken);
        logger.LogInformation("Claims worker RabbitMQ topology initialized");
    }
